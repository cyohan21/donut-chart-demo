...
const DUMMY_DATA: Record<string, Data[]> = {
  January: [
...
const generateWeeklyData = (currentMonthData: Data[], previousMonthData: Data[], monthName: string) => {
...
const DonutPath = ({
  ...
  const start = decimals.value.slice(0, index).reduce((a: number, b: number) => a + b, 0) + halfGap;
  const end = decimals.value.slice(0, index + 1).reduce((a: number, b: number) => a + b, 0) - halfGap;
...
const DonutChart = ({
  ...
  const smallFont = useFont(require('../../components/protected/donut/fonts/Roboto-Bold.ttf'), 40);
